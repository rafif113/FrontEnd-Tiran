...
const DetailVendorKendari = () => {
  ...
  const validation = useFormik({
    initialValues: {},
    onSubmit: async () => {
      const updatedFormData = detailVendorKendari.partrequests.map((row) => {
        const qty = values[`qty_${row.id}`];
        const description = values[`description_${row.id}`];

        return { qty, description };
      });

      // Lakukan apa pun yang perlu Anda lakukan dengan data ini (misalnya, kirim ke server)
